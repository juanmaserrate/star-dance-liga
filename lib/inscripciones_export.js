...
async function buildXlsxOrdenDePista(rows, tournament) {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Liga Star Dance';
  wb.created = new Date();
  ...
  // A: para anotar a mano · B: horario · C a G: los datos de la planilla
  const anchos = [5, 9, 26, 22, 8, 28, 14];
  anchos.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
  const COLS = anchos.length;

  const merge = (r, desde, hasta, height, value, font, fill, align, indent) => {
    ws.mergeCells(r, desde, r, hasta);
    const cell = ws.getCell(r, desde);
    cell.value = value;
    cell.font = font;
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
    cell.alignment = {
  ...
  // Encabezado: nombre del torneo y su fecha, como el título de la planilla
  const titulo = [tournament.name, tournament.datesLabel].filter(Boolean).join('  ·  ');
  merge(row++, 1, COLS, 30, titulo,
    { bold: true, size: 14, color: { argb: GOLD_LIGHT }, name: 'Arial' }, VIOLET);

  const d = new Date();
  const fecha = String(d.getDate()).padStart(2, '0') + '/' +
    String(d.getMonth() + 1).padStart(2, '0') + '/' + d.getFullYear();
  const sede = [tournament.venue, rows.length + ' inscripciones', 'Generado el ' + fecha]
    .filter(Boolean).join('  ·  ');
  merge(row++, 1, COLS, 18, sede,
    { italic: true, size: 9, color: { argb: VIOLET_DARK }, name: 'Arial' }, GOLD);

  if (!rows.length) {
    merge(row, 1, COLS, 24, 'Este torneo todavía no tiene inscripciones.',
      { bold: true, size: 11, color: { argb: VIOLET_DARK }, name: 'Arial' }, CREAM);
    return wb.xlsx.writeBuffer();
  }

  const encabezados = ['Apellido', 'Nombre', 'Edad', 'Institución', 'Salida a pista'];

  for (const bloque of groupByOrdenDePista(rows)) {
    // Franja violeta con el nombre completo del bloque. La celda del horario
    // queda vacía a propósito: no hay horarios cargados en el sistema.
    ws.getCell(row, 1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: VIOLET } };
    const horario = ws.getCell(row, 2);
    horario.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: VIOLET } };
    horario.border = boxGold;
    merge(row, 3, COLS, 24, bloque.titulo,
      { bold: true, size: 12, color: { argb: GOLD_LIGHT }, name: 'Arial' }, VIOLET, 'left', 1);
    row++;

    // Encabezado del cuadro
    [1, 2].forEach(c => {
      ws.getCell(row, c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: VIOLET_SOFT } };
    });
    encabezados.forEach((h, i) => {
      const cell = ws.getCell(row, i + 3);
      cell.value = h;
      cell.font = { bold: true, italic: true, size: 10, color: { argb: WHITE }, name: 'Arial' };
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: VIOLET_SOFT } };
    ...
    bloque.filas.forEach((r, i) => {
      const fill = i % 2 === 0 ? WHITE : CREAM;

      // Columna en blanco para anotar el puesto a mano, y la del horario.
      [1, 2].forEach(c => {
      ...
      if (r.is_group) {
        // Grupos y parejas: el nombre de fantasía ocupa Apellido y Nombre, con
        // sus integrantes debajo, en la misma celda.
        const detalle = r.integrantes && r.integrantes.length
          ? (r.group_name || 'GRUPO') + '\n' + r.integrantes.join(' · ')
          : (r.group_name || 'GRUPO');
        merge(row, 3, 4, 28, detalle,
          { size: 10, color: { argb: VIOLET_DARK }, name: 'Arial' }, fill, 'left');
        ws.getCell(row, 4).border = boxGold;
        ws.getCell(row, 3).border = boxGold;
      } else {
        [r.apellido || '', r.nombre_pila || ''].forEach((v, ci) => {
          const cell = ws.getCell(row, ci + 3);
          cell.value = v;
          cell.font = { size: 10, color: { argb: VIOLET_DARK }, name: 'Arial' };
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
      ...
      const edad = ws.getCell(row, 5);
      edad.value = r.edad === null || r.edad === undefined ? '' : Number(r.edad);
      edad.font = { size: 10, color: { argb: VIOLET_DARK }, name: 'Arial' };
      edad.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
      edad.alignment = { horizontal: 'center', vertical: 'middle' };
      edad.border = boxGold;

      const institucion = ws.getCell(row, 6);
      institucion.value = r.club || '';
      institucion.font = { size: 10, color: { argb: VIOLET_DARK }, name: 'Arial' };
      institucion.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
      institucion.alignment = { horizontal: 'left', vertical: 'middle' };
      institucion.border = boxGold;

      // Salida a pista: queda en blanco. El orden de salida lo define la
      // organización el día del torneo, no es un dato que tengamos cargado.
      const salida = ws.getCell(row, 7);
      salida.font = { size: 10, color: { argb: VIOLET_DARK }, name: 'Arial' };
      salida.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
      salida.alignment = { horizontal: 'center', vertical: 'middle' };
      salida.border = boxGold;
...
module.exports = {
  ...
  groupByOrdenDePista, buildXlsxOrdenDePista,
  sheetName
};
