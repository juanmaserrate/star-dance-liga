...
const EXPORT_FIELDS = [
  ...
  { id: 'ARANCEL_BASE',      label: 'Arancel Base' },
  { id: 'BONIFICACION',      label: 'Bonificación' },
  { id: 'ARANCEL_FINAL',     label: 'Arancel Final' },
  { id: 'ESTADO_PAGO',       label: 'Estado de Pago' }
];
...
const FIELD_WIDTHS = {
  ...
  SEGURO_OBRA_SOCIAL: 22, NRO_POLIZA: 18, ARANCEL_BASE: 14, BONIFICACION: 14,
  ARANCEL_FINAL: 14, ESTADO_PAGO: 16
};
...
function buildQuery(filters = {}) {
  ...
  let query = `
    SELECT
      ...
      r.discount_amount AS "BONIFICACION",
  ...
  if (filters.payment_status) { query += ` AND r.payment_status = ?`; params.push(filters.payment_status); }
...
function fieldValue(row, id) {
  switch (id) {
    ...
    case 'ARANCEL_BASE': return row.ARANCEL_BASE === null || row.ARANCEL_BASE === undefined ? '' : Number(row.ARANCEL_BASE);
    case 'BONIFICACION': return row.BONIFICACION === null || row.BONIFICACION === undefined ? '' : Number(row.BONIFICACION);
    case 'ARANCEL_FINAL': return row.ARANCEL_FINAL === null || row.ARANCEL_FINAL === undefined ? '' : Number(row.ARANCEL_FINAL);
    case 'ESTADO_PAGO': return String(row.ESTADO_PAGO || '').toUpperCase() === 'PAID' ? 'PAGADO' : 'PENDIENTE';
    default: {
      const v = row[id];
...
async function buildXlsx(rows, fields, meta) {
  ...
  const CREAM = 'FFF8F0DE', WHITE = 'FFFFFFFF', TEAL = 'FF4DD0C3', ORANGE = 'FFFFB74D';
  ...
  rows.forEach((row, ri) => {
    ...
    fields.forEach((f, ci) => {
      ...
      if (typeof val === 'number') {
        if (f.id === 'ARANCEL_BASE' || f.id === 'BONIFICACION' || f.id === 'ARANCEL_FINAL') {
          cell.numFmt = '"$"#,##0.00';
        } else {
          cell.numFmt = '0';
        }
        cell.alignment = { horizontal: 'right', vertical: 'middle' };
      } else if (f.id === 'ESTADO_PAGO') {
        const paid = val === 'PAGADO';
        cell.font = { bold: true, size: 10, color: { argb: paid ? TEAL : ORANGE }, name: 'Arial' };
...
function buildMeta(filters = {}, count) {
  ...
  if (filters.payment_status) parts.push(filters.payment_status === 'paid' ? 'Solo pagados' : 'Solo pendientes');
