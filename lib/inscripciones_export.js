...
async function fetchForGroupedSheet(tournamentId) {
  ...
  // Franjas configuradas del torneo, para poder subdividir por edad incluso en
  // las disciplinas que no las tienen cargadas.
  const bandRows = await db.prepare(`
    SELECT discipline, name, min_age, max_age FROM tournament_age_bands
    WHERE tournament_id = ? ORDER BY discipline ASC, order_index ASC, min_age ASC
  `).all(tournamentId);
  const bandsByDiscipline = {};
  for (const b of bandRows) {
    if (!bandsByDiscipline[b.discipline]) bandsByDiscipline[b.discipline] = [];
    bandsByDiscipline[b.discipline].push({ name: b.name, min: b.min_age, max: b.max_age });
  }

  return completarFranjas(rows, bandsByDiscipline);
}
...
// Reproduce la planilla que se usa en la pista: un bloque por cada categoria
// subdividida por franja etaria, y adentro las patinadoras con Apellido,
// Nombre, Institucion, salida a pista y las columnas de los jueces.
//
// Lo que NO se rellena, por pedido expreso: el horario y la salida a pista.
// Ambas columnas quedan en blanco para completar a mano.

// Franja que le corresponde a una edad dentro de un juego de franjas.
function franjaDeEdad(bandas, edad) {
  const n = Number(edad);
  if (!Number.isFinite(n) || !Array.isArray(bandas)) return null;
  const b = bandas.find(x => n >= x.min && n <= x.max);
  return b ? b.name : null;
}

// Completa la franja de cada inscripcion. Manda la guardada; si no hay, se
// deduce de la edad con las franjas de esa disciplina y, si la disciplina no
// tiene ninguna configurada (LIBRE), con el juego mas completo del torneo.
// Asi la planilla puede subdividir "3C CADETE" aunque LIBRE no use franjas.
function completarFranjas(rows, bandsByDiscipline) {
  const porDisciplina = bandsByDiscipline || {};
  const respaldo = Object.values(porDisciplina)
    .reduce((mejor, lista) => (lista && lista.length > (mejor ? mejor.length : 0) ? lista : mejor), null);

  return rows.map(r => {
    const guardada = String(r.categoria_edad || '').trim();
    if (guardada) return { ...r, franja: guardada, franjaDeducida: false };

    const propias = porDisciplina[r.discipline];
    const deducida = franjaDeEdad(propias && propias.length ? propias : respaldo, r.edad);
    return { ...r, franja: deducida || '', franjaDeducida: !!deducida };
  });
}

// Agrupa por disciplina + categoria + franja, respetando el orden configurado.
function groupByOrdenDePista(rows) {
  ...
  for (const r of rows) {
    const label = formatCategoryName(r.category_name, r.discipline);
    const franja = String(r.franja !== undefined ? r.franja : (r.categoria_edad || '')).trim();
    const clave = [r.discipline, label, franja.toUpperCase()].join('||');

    if (!porClave.has(clave)) {
      // En la planilla LIBRE va sin prefijo (es la disciplina por defecto);
      // el resto se nombra con su disciplina adelante.
      const prefijo = r.discipline === 'LIBRE' ? '' : r.discipline + ' ';
      const bloque = {
        discipline: r.discipline,
        label,
        franja,
        titulo: (prefijo + label + (franja ? ' ' + franja : '')).toUpperCase().trim(),
  ...
  bloques.sort((a, b) =>
    ...
    (edadMinima(a) - edadMinima(b)) ||
    a.franja.localeCompare(b.franja, 'es'));

  return bloques.map(b => ({ ...b, filas: porEdad(b.filas), total: b.filas.length }));
}

// Para ordenar las franjas de menor a mayor sin depender del nombre.
function edadMinima(bloque) {
  const edades = bloque.filas
    .map(f => Number(f.edad))
    .filter(n => Number.isFinite(n));
  return edades.length ? Math.min(...edades) : 999;
}
...
async function buildXlsxLibroMayor(rows, tournament) {
  ...
  // Paleta y tipografia de la planilla original
  const AMARILLO = 'FFFFFF00', VIOLETA = 'FF7030A0', CELESTE = 'FF00B0F0';
  const BLANCO = 'FFFFFFFF', NEGRO = 'FF000000';
  const FUENTE = 'Times New Roman';
  const linea = { style: 'thin', color: { argb: NEGRO } };
  const marco = { top: linea, left: linea, bottom: linea, right: linea };

  // A: horario · B-D: datos · E: salida a pista · F-H: jueces
  const anchos = [8.9, 20.6, 21.1, 29.8, 12.9, 8.2, 8.2, 8.2];
  anchos.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
  const COLS = anchos.length;

  const pintar = (celda, fill, font, align) => {
    if (fill) celda.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
    celda.font = { name: FUENTE, ...font };
    celda.alignment = { vertical: 'middle', ...(align || {}) };
    celda.border = marco;
    return celda;
  };
  ...
  // Titulo del torneo, en la franja amarilla
  ws.mergeCells(row, 1, row, COLS);
  pintar(ws.getCell(row, 1),
    AMARILLO, { size: 12, bold: true, color: { argb: NEGRO } },
    { horizontal: 'center' }).value =
    [tournament.name, tournament.datesLabel].filter(Boolean).join('  ·  ');
  ws.getRow(row).height = 14.25;
  row++;

  if (!rows.length) {
    ws.mergeCells(row, 1, row, COLS);
    pintar(ws.getCell(row, 1), null, { size: 12, bold: true }, { horizontal: 'center' })
      .value = 'Este torneo todavía no tiene inscripciones.';
    return wb.xlsx.writeBuffer();
  }

  const encabezados = ['Apellido', 'Nombre', 'Institución', 'Salida a pista', 'JUEZ 1', 'JUEZ 2', 'JUEZ 3'];

  for (const bloque of groupByOrdenDePista(rows)) {
    // Franja violeta con el nombre del bloque
    for (let c = 1; c <= COLS; c++) {
      pintar(ws.getCell(row, c), VIOLETA, { size: 14, bold: true, color: { argb: BLANCO } });
    }
    ws.getCell(row, 2).value = bloque.titulo;
    ws.getRow(row).height = 14.25;
    row++;

    // Encabezado del cuadro. La celda del horario queda vacia a proposito.
    pintar(ws.getCell(row, 1), CELESTE, { size: 14, bold: true }, { horizontal: 'center' });
    encabezados.forEach((h, i) => {
      pintar(ws.getCell(row, i + 2), VIOLETA,
        { size: 12, bold: true, color: { argb: BLANCO } }, { horizontal: 'center' }).value = h;
    });
    ws.getRow(row).height = 14.25;
    row++;

    // Cada patinadora ocupa dos renglones combinados, como en la planilla
    bloque.filas.forEach(r => {
      const nombre = r.is_group
        ? (r.group_name || 'GRUPO')
        : (r.apellido || '');
      const pila = r.is_group
        ? (r.integrantes && r.integrantes.length ? r.integrantes.join(' · ') : '')
        : (r.nombre_pila || '');

      [1, 2, 3, 4, 5].forEach(c => { ws.mergeCells(row, c, row + 1, c); });

      pintar(ws.getCell(row, 1), null, { size: 12 }, { horizontal: 'center' });
      pintar(ws.getCell(row, 2), null, { size: 12, color: { argb: NEGRO } }, { horizontal: 'center', wrapText: true }).value = nombre;
      pintar(ws.getCell(row, 3), null, { size: 12, color: { argb: NEGRO } }, { horizontal: 'center', wrapText: true }).value = pila;
      pintar(ws.getCell(row, 4), null, { size: 12, color: { argb: NEGRO } }, { horizontal: 'center', wrapText: true }).value = r.club || '';
      // Salida a pista: vacia, la completa la organizacion
      pintar(ws.getCell(row, 5), null, { size: 14, bold: true }, { horizontal: 'center' });
      // Columnas de los jueces, sin combinar, para escribir a mano
      [6, 7, 8].forEach(c => {
        pintar(ws.getCell(row, c), null, { size: 12 }, { horizontal: 'center' });
        pintar(ws.getCell(row + 1, c), null, { size: 12 }, { horizontal: 'center' });
      });

      ws.getRow(row).height = 17.25;
      ws.getRow(row + 1).height = 17.25;
      row += 2;
    });
  }
...
module.exports = {
  ...
  groupByOrdenDePista, buildXlsxLibroMayor, completarFranjas, franjaDeEdad,
