...
// --- Planilla de orden de salida a pista ------------------------------------
...
async function buildXlsxOrdenDePista(rows, tournament) {
  ...
  const ws = wb.addWorksheet('Orden de Pista', {
    views: [{ state: 'frozen', ySplit: 2 }],
    pageSetup: { paperSize: 9, orientation: 'portrait', fitToPage: true, fitToWidth: 1, fitToHeight: 0 }
  });

  // A: para anotar a mano · B: horario · C a F: los datos de la planilla
  const anchos = [5, 9, 26, 22, 28, 14];
  ...
  const encabezados = ['Apellido', 'Nombre', 'Institución', 'Salida a pista'];
  ...
  for (const bloque of groupByOrdenDePista(rows)) {
    ...
    bloque.filas.forEach((r, i) => {
      ...
      const institucion = ws.getCell(row, 5);
