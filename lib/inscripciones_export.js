...
async function buildXlsxPorCategoria(rows, tournament) {
  ...
  for (const disc of groupByDisciplineAndCategory(rows)) {
    ...
    for (const cat of disc.categorias) {
      ...
      ['NOMBRE COMPLETO', 'EDAD', 'CATEGORÍA', 'CATEGORÍA DE EDAD', 'CLUB'].forEach((h, i) => {
      ...
      cat.filas.forEach((r, i) => {
        ...
        // La categoría también va como dato de cada renglón, además de titular
        // el bloque, para que el cuadro se pueda leer o filtrar por sí solo.
        const valores = [
          nombre,
