...
async function ensureCategoriesCatalog({ force = false } = {}) {
  ...
  for (const t of tournaments) {
    ...
    for (const cat of existing) {
      ...
      if (inCatalog) {
        await db.prepare(`UPDATE categories SET is_active = true, order_index = ?, ruleset = ? WHERE id = ?`)
          .run(categoryOrder(cat.discipline, cat.name),
            tc.rulesetFor(cat.discipline, formatCategoryName(cat.name, cat.discipline)),
            cat.id);
        continue;
      }
    ...
    const insertIfMissing = async (discipline, catName, orderIndex) => {
      ...
      if (!dup) {
        ...
        `).run(t.id, displayName, discipline, catName, orderIndex,
          tc.rulesetFor(discipline, catName));
        added++;
      }
    };
