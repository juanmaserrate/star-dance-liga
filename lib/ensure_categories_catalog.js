...
async function ensureCategoriesCatalog({ force = false } = {}) {
  ...
  for (const t of tournaments) {
    ...
    for (const cat of existing) {
      ...
      if (inCatalog || isExtraCategory(extras, cat.discipline, cat.name)) {
        await db.prepare(`UPDATE categories SET is_active = true WHERE id = ?`).run(cat.id);
        continue;
      }
      ...
      if (referencedIds.has(String(cat.id))) {
        ...
        await db.prepare(`UPDATE categories SET is_active = false WHERE id = ?`).run(cat.id);
        kept++;
        continue;
      }
