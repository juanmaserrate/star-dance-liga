...
const CATALOG_REVISION = '2026-08-16-adultos';
...
// Reglamentos de una disciplina (solo ADULTOS los usa hoy). Devuelve [] si la
// disciplina no se divide por reglamento.
function catalogRulesets(discipline) {
  const group = catalogGroup(discipline);
  return Array.isArray(group && group.rulesets) ? group.rulesets : [];
}

// Reglamento al que pertenece una categoría del catálogo, o null.
function rulesetFor(discipline, categoryName) {
  const target = normalize(categoryName);
  const found = catalogRulesets(discipline)
    .find(r => (r.categories || []).some(cn => normalize(cn) === target));
...
async function seedCategories(tournamentId) {
  ...
  for (const group of catalog) {
    for (let i = 0; i < group.categories.length; i++) {
      const catName = group.categories[i];
      ...
      // order_index = posición en el catálogo, para que el desplegable respete
      // el orden oficial y no salga alfabético.
      await db.prepare(`
      ...
      `).run(tournamentId, displayName, group.discipline, catName, i,
        rulesetFor(group.discipline, catName));
      added++;
    }
  }
...
// Reglamentos realmente cargados en un torneo, agrupados por disciplina:
// { 'ADULTOS': ['REGLAMENTO INTERNO STAR DANCE', 'REGLAMENTO CAP'] }.
// Se leen de la base (no del catálogo) para que también aparezcan los que el
// administrador haya agregado a mano desde el panel del torneo.
async function getRulesetsByDiscipline(tournamentId) {
  const rows = await db.prepare(`
    SELECT discipline, ruleset, MIN(order_index) AS ord
    FROM categories
    WHERE tournament_id = ?
      AND ruleset IS NOT NULL AND ruleset <> ''
      AND COALESCE(is_active, true) = true
    GROUP BY discipline, ruleset
    ORDER BY discipline ASC, ord ASC, ruleset ASC
  `).all(tournamentId);

  const map = {};
  for (const r of rows) {
    if (!map[r.discipline]) map[r.discipline] = [];
    map[r.discipline].push(r.ruleset);
  }
  return map;
}
